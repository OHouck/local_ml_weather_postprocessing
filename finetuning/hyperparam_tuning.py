...
def optimize_hyperparameters_separate(regions_config: List[Dict[str, Any]], 
                                    architecture: str,
                                    max_evals: int = 100,
                                    output_dir: str = None,
...
def optimize_hyperparameters_combined(regions_config: List[Dict[str, Any]], 
                                     max_evals: int = 100,
                                     output_dir: str = None,
                                     device: torch.device = None,
                                     random_seed: int = 42) -> Dict[str, Any]:
    """
    Optimize hyperparameters for both MLP and UNet architectures simultaneously.
    Architecture selection is part of the optimization process.
    ...
    # Create output directory
    if output_dir is None:
        output_dir = f"hyperopt_results_combined_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(output_dir, exist_ok=True)
    ...
    def objective(hyperparams):
        ...
        print(f"\nEvaluating {arch.upper()} hyperparameters: {hyperparams}")
        result = evaluate_hyperparameters(hyperparams, regions_config, device, verbose=True)
        
        # Save intermediate results
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        result_file = os.path.join(output_dir, f'eval_{timestamp}_{arch}.json')
        with open(result_file, 'w') as f:
            json.dump({
                'hyperparams': hyperparams,
                'loss': result['loss'],
    ...
    # Prepare results
    results = {
        'best_architecture': best_hyperparams['architecture'],
        'best_hyperparams': best_hyperparams,
        'best_loss': best_trial['result']['loss'],
        'best_region_losses': best_trial['result'].get('region_losses', {}),
        'n_evaluations': len(trials.trials),
    ...
    # Separate results by architecture for analysis
    mlp_trials = [t for t in trials.trials if t['misc']['vals']['architecture'][0] == 0]  # 0 = mlp
    unet_trials = [t for t in trials.trials if t['misc']['vals']['architecture'][0] == 1]  # 1 = unet
    
    if mlp_trials:
        best_mlp_idx = np.argmin([t['result']['loss'] for t in mlp_trials])
        results['best_mlp_loss'] = mlp_trials[best_mlp_idx]['result']['loss']
        results['mlp_evaluations'] = len(mlp_trials)
    
    if unet_trials:
        best_unet_idx = np.argmin([t['result']['loss'] for t in unet_trials])
        results['best_unet_loss'] = unet_trials[best_unet_idx]['result']['loss']
        results['unet_evaluations'] = len(unet_trials)
    
    # Save final results
    results_file = os.path.join(output_dir, 'optimization_results_combined.json')
    with open(results_file, 'w') as f:
        json.dump({
            ...
            'best_region_losses': results['best_region_losses'],
            'n_evaluations': results['n_evaluations'],
            'best_mlp_loss': results.get('best_mlp_loss'),
            'best_unet_loss': results.get('best_unet_loss'),
            'mlp_evaluations': results.get('mlp_evaluations', 0),
            'unet_evaluations': results.get('unet_evaluations', 0)
        }, f, indent=2)
    
    # Save trials object for later analysis
    trials_file = os.path.join(output_dir, 'trials_combined.pkl')
    with open(trials_file, 'wb') as f:
        pickle.dump(trials, f)
    ...
    print(f"Best hyperparameters: {best_hyperparams}")
    print(f"Best loss: {results['best_loss']:.6f}")
    if 'best_mlp_loss' in results:
        print(f"Best MLP loss: {results['best_mlp_loss']:.6f} ({results.get('mlp_evaluations', 0)} evaluations)")
    if 'best_unet_loss' in results:
        print(f"Best UNet loss: {results['best_unet_loss']:.6f} ({results.get('unet_evaluations', 0)} evaluations)")
    print(f"Results saved to: {output_dir}")
...
def optimize_hyperparameters(regions_config: List[Dict[str, Any]], 
                           architecture: Union[str, List[str]] = "combined",
                           max_evals: int = 100,
                           output_dir: str = None,
                           device: torch.device = None,
                           random_seed: int = 42) -> Dict[str, Any]:
    """
    Main function to optimize hyperparameters. Can handle single architecture, 
    multiple architectures separately, or combined optimization.
    
    Args:
        regions_config: List of region configurations
        architecture: 'mlp', 'unet', ['mlp', 'unet'], or 'combined'
        max_evals: Maximum number of evaluations (per architecture if separate)
        output_dir: Directory to save results
        device: torch device (if None, will auto-detect)
        random_seed: Random seed for reproducibility
    ...
    if architecture == "combined":
        # Optimize both architectures in single search space
        return optimize_hyperparameters_combined(
        ...
        # Optimize single architecture
        return optimize_hyperparameters_separate(
            regions_config=regions_config,
            architecture=architecture,
            max_evals=max_evals,
            output_dir=output_dir,
        ...
        # Optimize multiple architectures separately
        all_results = {}
        
        for arch in architecture:
            print(f"\n{'='*50}")
            print(f"Optimizing {arch.upper()} architecture")
            print(f"{'='*50}")
            
            arch_output_dir = output_dir
            if output_dir is not None:
                arch_output_dir = os.path.join(output_dir, arch)
            
            results = optimize_hyperparameters_separate(
                regions_config=regions_config,
                architecture=arch,
                max_evals=max_evals,
        ...
        best_arch = min(all_results.keys(), key=lambda k: all_results[k]['best_loss'])
        
        combined_results = {
            'best_architecture': best_arch,
            'best_overall_loss': all_results[best_arch]['best_loss'],
            'best_hyperparams': all_results[best_arch]['best_hyperparams'],
            'results_by_architecture': all_results
        }
        ...
        if output_dir is not None:
            ...
            with open(summary_file, 'w') as f:
                json.dump({
                    'best_architecture': best_arch,
                    'best_overall_loss': combined_results['best_overall_loss'],
                    'best_hyperparams': combined_results['best_hyperparams'],
                    'loss_by_architecture': {k: v['best_loss'] for k, v in all_results.items()}
                }, f, indent=2)
        ...
        print(f"Best loss: {combined_results['best_overall_loss']:.6f}")
        for arch, results in all_results.items():
            print(f"{arch.upper()}: {results['best_loss']:.6f}")
        
        return combined_results
    
    else:
        raise ValueError(f"Invalid architecture specification: {architecture}")
...
if __name__ == "__main__":
    ...
    mlp_results = optimize_hyperparameters(
        ...
        architecture="mlp",
        max_evals=50,
        output_dir="hyperopt_results_mlp_only",
        device=device,
        random_seed=42
    )
    
    # Example 2: Optimize UNet only
    print("UNet optimization")
    unet_results = optimize_hyperparameters(
        regions_config=regions_config,
        architecture="unet",
        max_evals=50,
        output_dir="hyperopt_results_unet_only",
        device=device,
        random_seed=42
    )
    ...
    print(f"Best architecture from combined optimization: {unet_results['best_architecture']}")
    print(f"Best loss: {unet_results['best_loss']:.6f}")